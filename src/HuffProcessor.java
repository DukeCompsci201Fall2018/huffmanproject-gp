...
//		}
	...
	public int[] readForCounts(BitInputStream in) {
		...
		freq[in.readBits(BITS_PER_WORD)] = freq[in.readBits(BITS_PER_WORD)] + 1;
		}
		return freq;
	}
...
//		}
	...
	public HuffNode readTreeHeader(BitInputStream in) {
		...
		if (bit == -1) {
			throw new HuffException("Illegal argument: numBits must be on [1, 32]");
		}
		if (bit == 0) {
	...
	public void readCompressedBits(HuffNode root, BitInputStream in, BitOutputStream out) {
		...
		while (true) {
			...
			if (bits == -1) {
				...
				if (bits == 0) {
					current = current.myLeft; // go left
				} 
				else {
					current = current.myRight; // go right
				}
				...
				if (current.myLeft == null && current.myRight == null) { // reached a leaf!
					if (current.myValue == PSEUDO_EOF) {
						...
						current = root; // start back at the root after reaching leaf
					}
				}
			}
		}
